...
impl Solution {
    pub fn smallest_divisor(nums: Vec<i32>, threshold: i32) -> i32 {
        let (mut l, mut r) = (1, *nums.iter().max().unwrap() + 1);
        while l < r {
            let m = l + (r - l) / 2;
            let mval = nums.iter()
                .map(|&x| (x + m - 1) / m)
    ...
    pub fn smallest_divisor2(nums: Vec<i32>, threshold: i32) -> i32 {
        let (mut l, mut r) = (1, 10i32.pow(6)+1);
        while l < r {
            let m = l + (r - l) / 2;
            let mval = nums.iter()
                .map(|&x| x / m + if x % m == 0 { 0 } else { 1 })
                .sum::<i32>();
            //println!("mval={}, m={}", mval, m);
            if mval <= threshold {
                r = m;
            } else {
                l = m + 1;
